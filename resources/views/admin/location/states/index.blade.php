@extends('admin.layouts.dashboard', ['page_title' => "States of " . $country->name, "back_url" => route('admin.location.countries.index')])
...
@section('breadcrumb')
    ...
    <span class="kt-subheader__breadcrumbs-separator"></span>
    <span class="kt-subheader__breadcrumbs-link active">States</span>
@endsection
...
@push('modals')
    @include('admin.location.states.modals.create')
    @include('admin.location.states.modals.edit')
    @include('admin.location.states.modals.delete')
@endpush
...
@push('scripts')
    <script>
        $('#editModal').on('show.bs.modal', function (event) {
            ...
            var modal = $(this);
            modal.find('form').attr('action', "{{url('admin/location/countries/' . $country->id . '/states')}}/" + id );
        ...
        $('#deleteModal').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this);
            modal.find('.modal-footer form').attr('action', "{{url('admin/location/countries/' . $country->id . '/states')}}/" + id );
        });
            ...
            var jsonResource = function () {
                var datatable = $('.kt-datatable').KTDatatable({
                    ...
                    data: {
                        ...
                        source: {
                            read: {
                                url: '{{route('admin.location.countries.states.json', [$country->id])}}',
                                method: 'POST',
                                headers: {
                    ...
                    columns: [
                        {
                            ...
                            template: function(row) {
                                return '\
						<a href="/admin/location/countries/{{$country->id}}/states/'+row.id+'/cities" class="btn btn-sm btn-clean">\
                            Cities\
                        </a>\
