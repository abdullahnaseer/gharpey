@extends('admin.layouts.app', ['page_title' => "Cities of " . $state->name . ", " . $country->name, "back_url" => route('admin.location.countries.states.index', [$country->id])])
    ...
    <table class="table table-bordered dt-responsive nowrap">
        ...
        @foreach($records as $record)
            <tr>
                ...
                <td>{{$record->name}}</td>
                <td>
                    <a href="{{route('admin.location.countries.states.cities.areas.index', [$country->id, $state->id, $record->id])}}" class="btn btn-primary">
                        Areas
                    </a>
...
@push('modals')
    @include('admin.location.cities.modals.create', ['country' => $country, 'state' => $state])
    @include('admin.location.cities.modals.edit', ['country' => $country, 'state' => $state])
    @include('admin.location.cities.modals.delete', ['country' => $country, 'state' => $state])
@endpush
...
@push('scripts')
    <script>
        $('#deleteModal').on('show.bs.modal', function (event) {
            ...
            var modal = $(this);
            modal.find('.modal-footer form').attr('action', "{{url('admin/location/countries/' . $country->id . '/states/' . $state->id . '/cities')}}/" + id );
        });
        ...
        $('#editModal').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this);
            modal.find('form').attr('action', "{{url('admin/location/countries/' . $country->id . '/states/' . $state->id . '/cities')}}/" + id );
